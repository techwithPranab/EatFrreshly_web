...
import { analytics, pageTracking, businessEvents } from '@/utils/analytics';
...
const ClientPromotions = () => {
  ...
  const fetchPromotions = async () => {
    try {
      ...
      if (response.data.success) {
        const promos = response.data.data.promotions;
        setPromotions(promos);

        // Track promotions page view and impression count
        try {
          pageTracking.promotionsPage(promos.length);
        } catch (err) {
          console.warn('Analytics error (promotions page):', err);
        }
      }
    } catch (error: any) {
  ...
  // Track when a user copies or applies a promo code
  const copyPromoCode = (promotion: Promotion) => {
    const promoCode = promotion.promoCode;
    navigator.clipboard.writeText(promoCode);
    toast.success(`Promo code "${promoCode}" copied to clipboard!`);

    // Analytics: user selected/applied a promotion
    try {
      analytics.selectPromotion(promotion);
      businessEvents.applyPromoCode(promoCode, true, promotion.discountPercent);
    } catch (err) {
      console.warn('Analytics error (copyPromoCode):', err);
    }
  };
                    ...
                    <div className="p-6">
                      ...
                      <div className="bg-gray-50 rounded-lg p-3 mb-4">
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            onClick={() => copyPromoCode(promotion)}
